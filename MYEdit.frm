...
' ---------------------- 制御コマンド
  For i = 0 To 200
    ...
    Case "S"
      ...
      itp = itp + 1
      If itp < 7 Then
        dt = sisub(i): DispText4 itp, dt, True     '温度１
        dt = sjsub(i): DispText5 itp, dt, True     '温度２
        dt = sksub(i): DispText6 itp, dt, True     '昇温時間
      End If
    Case "L"
      ilp = 7
...
'
   ...
   dt = Z3_HoseiCont(2): DispText9 2 + 1, dt, True   'Z補正コントロール　　Z　No.
   For itk = 0 To 9
   If itk < T_keisuCont(0) Then
...
' ---------------------- 制御コマンド
  For i = 0 To 200
    ...
    Case "S"
      ...
      itp = itp + 1
      If itp < 7 Then
        sisub(i) = Val(Text4(itp - 1).Text)    '温度１
        sjsub(i) = Val(Text5(itp - 1).Text)    '温度２
        sksub(i) = Val(Text6(itp - 1).Text)    '昇温時間
      End If
    Case "L"
      ilp = 7
...
'
      ...
      Z3_HoseiCont(2) = Val(Text9(2).Text)    'Z補正ｃｏｎｔ　ポインター
      Henkou_No = Val(Text13.Text)    '変更内容         ----- 05.11.23
'
      For itk = 0 To 9
            T_keisu(itk) = Val(Text11(itk).Text)    '温度係数
      Next itk
      If T_keisuCont(1) > T_keisuCont(0) Then T_keisuCont(1) = T_keisuCont(0)
'
'
...
Private Sub Command3_Click(Index As Integer)
       If (iflgKataTorF(Index) = True) Then
          iflgKataTorF(Index) = False              '　ﾀﾞﾐｰは薄黄緑　false
          Command3(Index).BackColor = CmndColon(3)  'on 3=　薄緑
         Else
          iflgKataTorF(Index) = True
          Command3(Index).BackColor = CmndColoff(1)  '  off 1=灰色
       End If
End Sub
...
If (katamax = 6 Or katamax = 4) Then Text14(7).Visible = False
If (katamax = 6 Or katamax = 4) Then Text17(7).Visible = False
'
If katamax = 4 Then Text14(6).Visible = False
If katamax = 4 Then Text14(5).Visible = False
If katamax = 4 Then Text17(6).Visible = False
...
Private Sub DispText5(i%, dt!, flg%)    '　　温度２
  ...
  End If
End Sub
Private Sub DispText6(i%, dt!, flg%)    '　　昇温時間
  If flg = False Then
...
Private Sub DispText8(i%, dt!, flg%)    '　　待ち時間
  ...
  End If
End Sub
Private Sub DispText9(i%, dt!, flg%)    '　　温度係数cont（ポインター）
  If flg = False Then
...
Private Sub DispText14(i%, dt!, flg%)   '  型Ｎｏ． 調整数
  ...
  End If
End Sub
Private Sub DispText15(i%, dt!, flg%)   '  成形回数指定
  If flg = False Then
    VScroll15.Visible = False
    Text15.Visible = False
  Else
    VScroll15.Visible = True
    VScroll15.Value = dt * lK15
    Text15.Visible = True
    Text15.Text = Format(dt, "#0")
  End If
End Sub
...
' ---------------- 型個数　＆　ポインター
  For j = 0 To 1
    lK9 = 1
    VScroll9(j).min = 10 * lK9
    VScroll9(j).max = 0 * lK9
...
' ---------------- 肉厚補正
  For j = 0 To 9
    ...
    VScroll12(j).max = -0.5 * lK12           ' -0.05 →　-0.50　2012.5.13変更
...
' ---------------- 型Ｎｏ．　調整数
    ...
    ' ---------------- 成形回数指定
    lK15 = 1
    VScroll15.min = 6 * lK15
    VScroll15.max = 0 * lK15
    VScroll15.LargeChange = 1 * lK15
    VScroll15.SmallChange = 1 * lK15
End Sub
...
Dim dt!, inp%
  ...
  inp = Index + 1
  DispText9 inp, dt, True      '温度係数ｃｏｎｔ（ポインター）
End Sub
...
Dim dt!, inp%
  If lFlgDisp = False Then Exit Sub
  dt = VScroll15.Value / lK15
  inp = 1
  DispText15 inp, dt, True      '温度係数ｃｏｎｔ（ポインター）
End Sub
