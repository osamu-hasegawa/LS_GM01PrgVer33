...
'----------- ServoOn→位置制御→LSショート
  If BrdFlg <> "ON" Then Exit Sub
  ServoON
  DioOut 12, 1  'DioOut 27, 0    '位置制御
  'DioOut 24, 0    '速度制御
  'DioOut 29, 1    'CWストロークエンド→Driver
  '
  'DioOut 30, 1    'CCWストロークエンド→Driver
End Sub
Public Sub CtlVelo()
Dim disp!
'----------- ServoOn→速度制御→回転方向CW選択
  If BrdFlg <> "ON" Then Exit Sub
  ServoON
  DioOut 12, 0  'DioOut 27, 1    '速度制御
  'DioOut 24, 1    '速度制御
  'DioOut 29, 0    'CW→Driver
  '
  'DioOut 30, 1    'CCW→Driver
  disp = r_z()
End Sub
Public Sub N2Open()
'----------- 冷却バルブOPEN
...
Dim b0%, b1%, b2%, b3%, ErrNo%, EmgArm%, b7%, b8%
'-----------  非常停止メッセージ
  DioInput 7, b7     'EMG出力
  DioInput 8, b8     'ARM出力
...
'----------- 成形室 & 予熱室に型が在るか？　'04.9.26変更 s.f
  ...
  Else
    ...
    DioInput 19, sts2    '予熱室３ =1；型が在る
'                                           '06.12.21 change s.f
    If sts1 = 0 And sts2 = 1 Then sts = 1   '04.9.26 change　　予備室のみ型有り
    If sts1 = 1 And sts2 = 0 Then sts = 2   '04.9.26 change　　成形室のみ型有り
    If sts1 = 1 And sts2 = 1 Then sts = 3   '04.9.26 change　　成形室、予備室共に型有り
''    If sts1 = 1 And sts2 = 1 Then sts = 1   '04.9.26 change
'    If sts1 = 1 And sts2 = 0 Then sts = 1   '04.9.26 change　　成形室のみ型有り
'    If sts1 = 0 And sts2 = 1 Then sts = 2   '04.9.26 change　　予備室のみ型有り
'    If sts1 = 1 And sts2 = 1 Then sts = 3   '04.9.26 change　　成形室、予備室共に型有り
  End If
  KataChk = sts
End Function
